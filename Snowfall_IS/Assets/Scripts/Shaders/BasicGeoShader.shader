			...
			Pass
			{
				...
				CGPROGRAM
					...
					void geom(triangle v2g input[3], inout TriangleStream<g2f> TriStream) {
						...
						float4 p0 = UnityObjectToClipPos(input[0].worldPos + (input[0].normal) * _Factor * inf);
						
						inf = saturate(dot(worldNorm1, fixed3(0, 1, 0)));
						inf = inf < _SnowEdgeFactor ? 0 : inf;
						float4 p1 = UnityObjectToClipPos(input[1].worldPos + (input[1].normal) * _Factor * inf);
						
						inf = saturate(dot(worldNorm2, fixed3(0, 1, 0)));
						inf = inf < _SnowEdgeFactor ? 0 : inf;
						float4 p2 = UnityObjectToClipPos(input[2].worldPos + (input[2].normal) * _Factor * inf);
